...
import HandTrackingModule  # Importing custom HandTrackingModule
...
colors = [
    ...
    (196, 102, 255),
    (223, 192, 12),
    (1, 1, 1)        # Here (1, 1, 1) corresponds to eraser
]
...
while True:
    ...
    # Extracts the RGB channels and alpha channel (if present) from the image header, normalizing the alpha values, or assumes full opacity if no alpha channel exists.
    if header.shape[2] == 4:
    ...
    # Blend the region of img with overlay based on alpha transparency
    for c in range(3):
        img[y1:y2, x1:x2, c] = (
                overlay_alpha * overlay_bgr[:, :, c] + (1 - overlay_alpha) * img[y1:y2, x1:x2, c]
        )

    # Removing the alpha channel from an RGBA image (if present) and then converts the image to the uint8 data type
